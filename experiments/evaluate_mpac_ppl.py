...
def _load_metadata(comparison_dir: Path) -> dict[str, Any]:
    ...
    if metadata.get("baseline") != "MPAC":
        raise ValueError(f"{comparison_dir} does not look like an MPAC comparison directory")
    return metadata
...
def _experiment_dir_from_metadata(comparison_dir: Path, metadata: dict[str, Any]) -> Path:
    ...
    if not value:
        raise ValueError("MPAC metadata is missing experiment_dir")
    return _resolve_existing_path(str(value), base=comparison_dir)


def load_mpac_quality_examples(
    comparison_dir: Path,
    *,
...
def _load_runtime(
    *,
    ...
    if not generation_model:
        raise ValueError("MPAC metadata is missing model_path")
...
def score_mpac_ppl(
    *,
    ...
    if output_dir.exists() and any(output_dir.iterdir()) and not resume and not overwrite:
        raise FileExistsError(f"MPAC PPL output exists: {output_dir}. Use --resume or --overwrite.")
    ...
    examples = load_mpac_quality_examples(comparison_dir, include_shared=include_shared)
    ...
    write_json(
        output_dir / "metadata.json",
        {
            ...
            "mpac_ecc": metadata.get("mpac_ecc"),
            "generation_model": metadata.get("model_path"),
    ...
    processed, skipped = _score_examples(
        ...
        batch_size=batch_size,
        description=f"MPAC PPL [{display_name}]",
        evaluator_model_path=evaluator_model_path,
        evaluator_name=display_name,
        resume=resume,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Compute OPT-family PPL for existing MPAC comparison records."
    )
...
def _print_plan(args: argparse.Namespace) -> None:
    ...
    print("Generation:      disabled (existing MPAC texts only)")
    print("=" * 72)
