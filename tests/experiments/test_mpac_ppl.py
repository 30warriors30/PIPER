...
from experiments.evaluate_mpac_ppl import load_mpac_quality_examples, score_mpac_ppl
...
def _build_mpac_comparison(root: Path) -> Path:
    experiment_dir = root / "experiment"
    comparison_dir = experiment_dir / "comparisons" / "mpac_t200_b8_m2"
    write_json(
...
def test_load_mpac_quality_examples_uses_test_watermarked_and_shared_baseline(tmp_path: Path) -> None:
    comparison_dir = _build_mpac_comparison(tmp_path)

    examples = load_mpac_quality_examples(comparison_dir, include_shared=True)
...
def test_score_mpac_ppl_writes_quality_rows_and_summary(tmp_path: Path) -> None:
    comparison_dir = _build_mpac_comparison(tmp_path)
