...
def main() -> int:
    ...
    classes = (
        {"unwatermarked"}
        if args.negative_source == "unwatermarked"
        else (
            {"natural"}
            if args.negative_source == "natural"
            else {"unwatermarked", "natural"}
        )
    )
    scores = [
        float(record["z_score"])
        for record in records
        if record.get("input_mode") == args.input_mode
        and record.get("text_class") in classes
    ]
    print(f"Z-score calibration negatives: {len(scores)}")
    if args.dry_run:
